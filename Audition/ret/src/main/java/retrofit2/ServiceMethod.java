...
/**
 * 1 本类的构造函数
 * 2 toCall adapt response
 * 3 parsePath
 * 4 Builder
 */
public class ServiceMethod<T, R> {

    static final String PARAM = "[a-zA-Z][a-zA-Z0-9]*";
    static final Pattern PARAM_URL_REGEX = Pattern.compile("\\{(" + PARAM + ")\\}");
    static final Pattern PARAM_NAME_REGEX = Pattern.compile(PARAM);

    private final okhttp3.Call.Factory callFactory;
    private final CallAdapter<T, R> callAdapter;
    ...
    /** 在这里使用{@link RequestBuilder} Builds an HTTP request from method arguments. */
    ...
    /**
     * 获取给定URI中使用的惟一路径参数集。如果一个参数在URI中使用了两次，那么它只会在集合中出现一次。
    ...
    /**
     * 检查接口方法上的注释，以构造可重用的服务方法。
     * 这需要潜在的昂贵的反射，因此最好只构建一次并且重用它。Builder不能重用。
     *
     ...
     * 包括
     * 1 builder构造
     * 2 create callAdapter  converter
     * 3 parse method等多个接口
     * 4 工具 methodError  parameterError
     */
