...
package com.lq.he.demo.jetnews.data

import android.content.res.Resources
import androidx.ui.graphics.imageFromResource
import com.lq.he.demo.jetnews.R
import com.lq.he.demo.jetnews.model.Markup
import com.lq.he.demo.jetnews.model.MarkupType
import com.lq.he.demo.jetnews.model.Metadata
import com.lq.he.demo.jetnews.model.Paragraph
import com.lq.he.demo.jetnews.model.ParagraphType
import com.lq.he.demo.jetnews.model.Post
import com.lq.he.demo.jetnews.model.PostAuthor
import com.lq.he.demo.jetnews.model.Publication

/**
 * Define hardcoded posts to avoid handling any non-ui operations.
 */

val pietroCN = PostAuthor("彼得罗美极", "https://medium.com/@pmaggi")
val manuelCN = PostAuthor("提那Vivo", "https://medium.com/@manuelvicnt")
val florinaCN = PostAuthor(
    "弗洛里纳",
    "https://medium.com/@florina.muntenescu"
)
val joseCN =
    PostAuthor("何塞", "https://medium.com/@JoseAlcerreca")

val publicationCN = Publication(
    "Android开发者",
    "https://cdn-images-1.medium.com/max/258/1*u7oZc2_5mrkcFaxkXEyfYA@2x.png"
)
val paragraphsPost1CN = listOf(
    Paragraph(
        ParagraphType.Text,
        "努力使我们的Android应用程序更加模块化,我最后一个示例,其中包括一组点播功能组合在一个文件夹中:"
    ),
    Paragraph(
        ParagraphType.Text,
        "非常标准的设置，所有按需模块，在一个“功能”文件夹内;清洁."
    ),
    Paragraph(
        ParagraphType.Text,
        "这些模块包含在settings.gradle文件中:"
    ),
    Paragraph(
    ...
    Paragraph(
        ParagraphType.Text,
        "这些设置很好地解决了一个“小”问题:在Android Studio的Android视图中有一个名为features的空模块:"
    ),
    Paragraph(
        ParagraphType.Text,
        "我可以接受，但我更愿意从我的项目中删除这个空模块!"
    ),
    Paragraph(
...
val post1CN = Post(
    ...
    publication = publicationCN,
    metadata = Metadata(
        author = pietroCN,
        date = "August 02",
        readTimeMinutes = 1
    ),
    paragraphs = paragraphsPost1CN,
...
var postsCN = listOf(
    ...
    post4CN,
    post5CN,
    post1CN.copy(id = "post1CN"),
    post2CN.copy(id = "post2CN"),
    post3CN.copy(id = "post3CN"),
    post4CN.copy(id = "post4CN"),
    post5CN.copy(id = "post5CN")
)

fun getPostsWithImagesLoadedCN(posts: List<Post>, resources: Resources): List<Post> {
    return posts.map {
        it.copy(
